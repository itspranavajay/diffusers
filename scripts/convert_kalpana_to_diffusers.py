...
from diffusers import AutoencoderKL, KalpanaTransformer2DModel
...
parser.add_argument("--filename", default="kalpana.safetensors", type=str)
...
def main(args):
    ...
    if args.transformer:
        ...
        transformer = KalpanaTransformer2DModel(guidance_embeds=has_guidance)
        transformer.load_state_dict(converted_transformer_state_dict, strict=True)

        print(
            f"Saving Kalpana Transformer in Diffusers format. Variant: {'guidance-distilled' if has_guidance else 'timestep-distilled'}"
        )
